...
(function () {
  ...
  var COPY = {
    ...
    letter: {
      ...
      // ⚠ ВОПРОС НАЗЫВАЕТ ВЫБОР (правка владельца 28.08). «Почему именно это?» не называло
      // ничего — ни выбора, ни сравнения, — и рядом с соседним «Почему эта стратегия
      // сработает?» читалось как тот же вопрос другими словами. А собирают они разное:
      // здесь основание выбора (контроль ПР-2, сравнивается с разбором заявок и печатью),
      // там механизм результата (контроль АК-2). Формулировка того же вида, что у
      // рекомендаций: «Почему она, а не одна из перечисленных?».
      why:     { label: 'Почему именно это, а не что-то другое?' },
  ...
  M.variants = {
    ...
    // ⚠ ВТОРОЙ ТАКТ ЗДЕСЬ КОНЧИЛСЯ (решение владельца 28.08). Три вопроса про основную
    // рекомендацию стали тремя шагами диалога в js/scenes.js — как у Штерна. Верстак теперь
    // ровно веер альтернатив: записал — ответил — день пошёл дальше. Вместе с тактом ушли
    // gate по mainText/mainWhy, onCta с двойным «Ответить» и hideKept (признак второго
    // такта). Поля mainText/mainWhy/mainFrom в состоянии ОСТАЮТСЯ пустыми строками: записи
    // до 28.08 их несут, и сводки с судейским текстом читают оба места — состояние верстака
    // и новые колонки ответов (backend/code.js, v2MechText_ ветка variants).
    ...
    render: function (host, m, ctx) {
      // ⚠ ВТОРОГО ТАКТА ЗДЕСЬ БОЛЬШЕ НЕТ (решение владельца 28.08). Он показывал заход
      // Агеева и три подписанных поля — рекомендация, почему она, как к ней пришли. Теперь
      // это три шага диалога в js/scenes.js: реплика — окно ответа без подписи — реплика.
      // Вместе с тактом ушла и подсказка «Агеев угадывает первый записанный вариант»: гадать
      // больше нечего, он просто спрашивает про ту, которую участник придержал.
      var draw = function () {
  ...
  M.list = {
    ...
    // ⚠ ПРОЙДЕННЫЙ РАЗБОР ПОКАЗЫВАЕТСЯ ЛИСТОМ, А НЕ СТРОКОЙ ИТОГА (правка владельца 28.08).
    // «Шаг с вопросом про заявку, на которой настоит правление, должен быть на одном экране
    // с разобранными заявками — так человек будет видеть весь набор». Строка итога говорила
    // только числа, и чтобы ответить «за счёт чего возьмём», участник вспоминал состав
    // наизусть; вместо набора ему выносили одну карточку. Лист тот же, что в печати
    // правления (decisionDoc), поэтому форма у набора одна на весь день, и кружки «i» с
    // доводом заявки есть сразу. Строка итога выше не удалена: она остаётся коротким видом
    // для мест, где лист не к месту, — на экране дня её больше не рисует никто.
    lockedBubble: function (m, ctx) { return decisionDoc(m, ctx); },
    render: function (host, m, ctx) {
      var draw = function () {
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // ЛИСТ РЕШЕНИЙ — ОДНА ФОРМА НА ВЕСЬ ДЕНЬ (правка владельца 28.08)
  //
  // «Вообще сразу формировать в тот же документ, что мы показываем на шаге позже,
  // чтобы не было разных форм для участника». Этот лист рисовался только в печати
  // правления, а на шаге обмена участник видел одну карточку заявки — то есть один
  // и тот же набор в двух разных видах. Теперь строитель один, и его зовут оба:
  //   · печать правления — со блоком стратегий (ctx.board);
  //   · пройденный разбор заявок (M.list.lockedBubble) — без него.
  // Блок стратегий приходит из scenes.js и есть только у акта печати, поэтому
  // условие в самом листе: у разбора ctx.board пустой, и блок не рисуется.
  // Кружки «i» с доводом заявки — часть листа, значит есть в обоих местах сразу.
  // ═════════════════════════════════════════════════════════════════════════
  function decisionDoc(lm, ctx) {
    var out = '';
        ...
        if (ctx.board) {
          out += '<div class="mx-board"><div class="bl-col-head">' + ctx.esc(ctx.board.title) + '</div>' +
  ...
  M.seal = {
    ...
    // ⚠ ВОПРОС ОСТАЁТСЯ ВИДЕН У ПРОЙДЕННОГО ШАГА (правка владельца 28.08: «тут не надо
    // сворачивать предыдущий вопрос Агеева»). На экране перелимита печать остаётся рядом
    // (withPrev), но у пройденного шага рисовался только пузырь участника — вопрос, на
    // который он отвечал, не показывался нигде: в свёртке лежит монолог давления, а сам
    // вопрос принадлежит верстаку. Теперь он идёт над ответом, и редакция та же, что видел
    // участник (m.changed выбирает между probe и probeReturn — так же, как в живом виде).
    lockedBubble: function (m, ctx) {
      return ctx.speech(m.changed && ctx.probeReturn ? ctx.probeReturn : ctx.probe) + ctx.mine(m.why);
    },
    ...
    render: function (host, m, ctx) {
      ...
      var draw = function () {
        ...
        // ⚠ ОБА ВОПРОСА СТОЯТ НАД ДОКУМЕНТОМ (правка владельца 28.08). «Ну что, будем
        // презентовать это решение? / Финализируем?» рисовались ПОД листом, за чертой:
        // сначала участник читал документ, и только в самом низу его спрашивали. Теперь
        // Агеев говорит всё сразу — «давайте посмотрим», «я выписал стратегии», «будем
        // презентовать?» — и под этим лежит лист, на который смотрят с уже заданным
        // вопросом. Черта, отделявшая разбор от разговора, вместе с этим не нужна:
        // разговор больше не идёт после списка.
        // Оба блока речи рисуем ОДНОЙ репликой: подряд они дали бы имя «Кирилл Агеев»
        // дважды, а правило «один голос — одно имя» (21.08) этого не допускает.
        var say = ctx.before;
        if (m.confirmed == null && ctx.ask && ctx.before) {
          say = { who: ctx.before.who, note: ctx.before.note,
                  bubbles: (ctx.before.bubbles || []).concat(ctx.ask.bubbles || []) };
        }
        var h = ctx.speech(say);
        if (lm) { h += decisionDoc(lm, ctx); }
        if (m.confirmed != null) {
        ...
        host.innerHTML = h;
        // ⚠ КРУЖКИ «i» СЛУШАЕТ ДВИЖОК, А НЕ ЭТОТ ВЕРСТАК (правка 28.08). Тот же лист теперь
        // рисует и пройденный разбор заявок, а его собирает engine.js — там обработчика не
        // было бы. Слушатель один на колонку разговора (см. engine.js, .off-i); держать
        // здесь второй значило бы переключать раскрытие дважды за один щелчок.
  ...
  M.goal = {
    ...
    gate: function (m, ctx) {
      ...
      if (String(m.became).trim() === '') return '«' + COPY.goal.became.label + '» — обязательно.';
      // ⚠ СРОК ВНЕ ШКАЛЫ (правка 16.09, находка на прогонах волны 021). У человека это
      // ползунок с делениями 1…16, и выйти за них он не может физически. У модели на том
      // же месте поле, куда она пишет строку, — и две модели написали «2030», имея в виду
      // ГОД. Записалось 2030, а судья прочитал в материале «Срок: 2030 лет». Проверка
      // стоит здесь, а не в разборе ответа, потому что правило одно на обе стороны: то,
      // чего нельзя выбрать на шкале, не может быть и принято. Человеку эта ветка не
      // достанется никогда — ползунок других значений не отдаёт.
      var y = m.years;
      if (y !== null && y !== undefined && y !== '' &&
          (!isFinite(Number(y)) || Number(y) < 1 || Number(y) > YMAX || Number(y) % 1 !== 0)) {
        return 'Срок — целое число лет от 1 до 15 (верхнее деление шкалы — «15+»). ' +
               'Похоже, здесь назван год, а не число лет.';
      }
      return '';
    },
  ...
  // ⚠ ПИСЬМО ОФОРМЛЕНО ПИСЬМОМ (правка владельца 28.08). В листе «Мои ответы» это были
  // четыре пары «подпись — ответ», как у любого другого шага, — а это единственный шаг
  // дня, который и по лору письмо: Агеев просит «одно письмо, четыре пункта» и зачитает
  // его правлению. Форма взята у письма Агеева в кейсе (blockquote.letter): шапка с «От/
  // Кому/Дата/Тема», тело, подпись. Наоборот по адресату — от участника к Агееву, потому
  // что письмо ждёт он: «очень жду до конца дня», а подписывается под ним перед правлением
  // сам.
  // ⚠ ВРЕМЯ ПИСЬМА — КОНЕЦ ДНЯ, А НЕ ВРЕМЯ ШАГА (правка владельца 28.08). Шаг стоит на
  // 13:00 — это когда Агеев звонит «между встречами» и просит письмо «до конца дня».
  // Само письмо участник отправляет позже, поэтому 17:00, и расхождения с шапкой шага
  // здесь нет: там время звонка, тут время отправки.
  // Дата держится литералом потому, что это лист ответа, а не реплика, и подстановок в
  // нём нет; при смене дня в сценах поправить и тут.
  M.letter.answerHtml = function (m, ctx) {
    var nm = (ctx.pname && ctx.pname()) || '';
    var head = '<p><b>От:</b> ' + (nm ? ctx.esc(nm) : 'вы') +
      '<br /><b>Кому:</b> Кирилл Агеев' +
      '<br /><b>Дата:</b> понедельник, 9 февраля 2026, 17:00' +
      '<br /><b>Тема:</b> резюме стратегии «Искры»</p>';
    // Четыре пункта — тело письма: вопрос жирным, ответ за ним. Пустые не печатаем:
    // гейт требует все четыре, но лист читается и у незакрытого шага.
    var body = LETTER.map(function (f) {
      return String(m[f[0]] || '').trim() ? p(f[1], ctx.br(m[f[0]])) : '';
    }).join('');
    // Подпись — как у Агеева в кейсе, только своя: он закрывает письмо одной буквой,
    // здесь полное обращение, потому что письмо идёт от постороннего человека внутрь
    // компании, а не от своего к своему.
    var sign = '<p style="margin:14px 0 0;">С уважением,<br />' +
      (nm ? ctx.esc(nm) : 'ваш консультант по стратегии') + '<br />' +
      '<i>независимый консультант по стратегии</i></p>';
    return '<blockquote class="letter letter-mine">' + head + body + sign + '</blockquote>';
  };
