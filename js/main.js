...
(function () {
  ...
  window.imp.deviceReq = {
    ...
    // ⚠ «Шесть развёрнутых письменных ответов» устарело с маршрутом v4.4.f: шагов
    // двенадцать — пять свободных ответов и семь механик, где надо раскладывать
    // список на двадцать позиций и собирать связки. Число здесь обязано совпадать с
    // тем, что участник встретит, иначе запрет объясняется неправдой.
    tail: 'С телефона ассессмент не проходят: в нём развёрнутые письменные ответы и работа с большим списком на экране, и на маленьком экране это меряло бы терпение, а не мышление.'
  };
