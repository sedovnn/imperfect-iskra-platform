...
(function () {
  ...
  screens.briefA = function () {
    return '<div class="xnarrow">' +
      ...
      '<p class="section-lead">Дальше — пятьдесят описаний, по одному на экран. Каждое описывает ' +
      'один уровень одной способности. Ваша задача: сказать, о какой способности речь.</p>' +
      '<p class="section-lead">Сначала выбираете навык из пяти, потом одну из двух способностей ' +
      'внутри него. Определения всегда под рукой — справочник открывается кнопкой сверху и ' +
      'закрывать его не нужно.</p>' +
      '<ul class="xlist">' +
      '<li><b>Номера уровней и коды из описаний убраны.</b> Если что-то похожее попалось — ' +
      'это наш недосмотр, отметьте в заметке.</li>' +
      '<li><b>Если описание не ложится ни к одной способности</b> — так и скажите кнопкой ' +
      '«не могу выбрать». Это не пропуск, а самый ценный ответ: значит, описание не работает.</li>' +
      '<li><b>Если подходят две</b> — выберите основную, потом отметьте вторую. Пара, которая ' +
      'постоянно путается, — то, что мы и ищем.</li>' +
      '<li><b>Возвращаться назад можно.</b> Скорость не измеряется.</li>' +
      '</ul>' +
  ...
  screens.done = function () {
    ...
    // Финиш — одна строка. Здесь стояло ещё три абзаца: благодарность, объяснение
    // почему не показываем правильные ответы, и рассказ о том, что мы будем
    // делать со сводкой. Всё это — про нас, а не про человека, который только
    // что отдал час и хочет знать одно: дошло или нет. Остаются номер (по нему
    // возвращаются) и предупреждение о неотправленном (иначе работа пропадёт) —
    // они не текст, а действие.
    return '<div class="xnarrow">' +
      '<h1>Ваши ответы записаны</h1>' +
      '<p class="xbadge">Ваш номер <b>' + esc(S.id) + '</b></p>' +
      (unsent
