...
(function () {
  ...
  // Имя спрашивается на экране установки (intro.html), а не здесь: выдумывать
  // персонажу повод знакомиться заново после звонка, пакета и письма было незачем,
  // а «внести в список на входе» просит полные данные, а не «Алекс». Имя приходит
  // в сессии и подставляется в реплики раундов 2–5; если его нет, реплики
  // обходятся без обращения (см. pname() в каждом раунде).
  ...
  // Счётчика в «Главной проблеме» нет: слот один, число могло быть только 0 или 1 —
  // это не информация. У соседних блоков число осмысленно (сколько заметок, сколько
  // связок), здесь нет. Пока не сформулировано, чип держит разрешение не отвечать;
  // как только формулировка появилась, чип уходит — она видна и так, в свёрнутом
  // виде и в развёрнутом. Обновляется точечно: полный renderDeep на каждый символ
  // пересобирал бы текстареа и сбрасывал каретку.
  function syncDeepChip() {
    var chip = document.getElementById('deepChip');
    if (!chip) return;
    var named = deepList().some(function (d) { return (d.text || '').trim(); });
    chip.textContent = named ? '' : 'необязательно';
    chip.className = 'wchip' + (named ? ' is-hidden' : '');
  }
  ...
  function renderDeep() {
    ...
    // Счётчика здесь нет: слот один, и число могло быть только 0 или 1 — это не
    // информация. У соседних блоков число осмысленно (сколько заметок, сколько
    // связок), здесь — нет. Пока не сформулировано, чип держит разрешение
    // не отвечать; как только формулировка есть, чип уходит совсем: она видна
    // и в свёрнутом виде, и в развёрнутом, повторять нечем.
    syncDeepChip();
  ...
  document.addEventListener('input', function (e) {
    ...
    if (t.hasAttribute('data-deep-text')) {
      ...
      // и убирает чип «необязательно» — формулировка уже есть
      if (wasEmpty !== !d.text.trim()) { syncAllCardActions(); syncDeepChip(); }
      return;
    }
