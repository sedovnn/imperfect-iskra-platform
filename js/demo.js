...
(function () {
  ...
  var PROFILES = {
    strong: {
      ...
      roomAlternatives: {
        answer1: 'Как велено — не сработает, и вот почему я бы не спешил с бюджетом. Деньги догоняют оффер, но не убирают причину смотреть наружу. Я бы держал в голове три разных хода: перебить офферы точечно только носителям экспертизы (быстро, но лечит симптом), дать команде продукт и автономию — то, за чем реально уходят в Nord Labs (медленно, но бьёт в причину), и честно отпустить часть, застраховав знания парными ролями. И сначала спросил бы: а почему они вообще переписываются? Если ответ «некуда расти» — это не про офферы совсем.',
        source: '',
    ...
    mid: {
      ...
      roomAlternatives: {
        answer1: 'Сработает, если деньги дать быстро — люди сравнивают офферы. Ещё можно попробовать удержать самых ключевых персональными условиями, если бюджета на всех не хватит.',
        source: '',
    ...
    weak: {
      ...
      roomAlternatives: { answer1: 'Думаю, сработает — деньги все любят.', source: '', sourceElaboration: '' },
  ...
  function seed(profile) {
    ...
    put('imp_round5_' + bib, {
      // v:2 обязателен: loadState раунда 5 сбрасывает незавершённые стейты без него
      // (миграция редизайна 2026-07-27 — см. js/round5.js)
      v: 2,
