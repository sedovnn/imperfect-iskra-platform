  ...
  function initWorkspace(s1cards) {
    ...
    function buildBacklogBlock() {
      ...
      block.innerHTML =
        ...
        them('', { speech: 'Но это ещё не всё. Правление поручило мне свести портфель изменений по всей группе — платит за него в основном реклама, значит и разбираться мне. Собрали со всех блоков, включая корпоративный центр: двадцать решений. Писали руководители, каждый со своего участка, и не все они мои. Часть, возможно, подходит под вашу стратегию, часть, наверное, придётся делать при любой — но это вам и решать. В общем и целом, мы ждём от вас рекомендации по поводу того, что нам приоритизировать на ближайший год.' }) +
    ...
    // Поручение на финальный документ выдаётся ЗДЕСЬ, в последней реплике. Без него
    // трое персонажей в раундах 3–5 ссылались на задание, которого никто не давал:
    // «вам пора за черновик к комитету», «на финализацию стратегии времени немного»,
    // «вас финализация стратегии заждалась». Одно место закрывает четыре.
    function buildProactiveBlock() {
      ...
      block.innerHTML =
        ...
        (locked
          ? them('Кирилл Агеев', { act: 'провожает вас до двери',
              speech: 'Тогда так. К вечеру сведите всё, что мы сегодня наговорили, в один документ — с ним я иду на правление. И по дороге посмотрите, что мы понесём на мартовский комитет: вопрос там будет тот же.' })
          : '') +
