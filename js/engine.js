...
(function () {
  ...
  // ⚠ СУДЕЙСТВО ПО ХОДУ ДНЯ (правка 18.09, вопрос владельца: «что нам мешает отправлять
  // ответ каждый раз, когда участник нажимает дальше?»). Ничего не мешало. Каждое задание
  // судьи читает свой набор окон, и восемь из тринадцати наборов закрываются задолго до
  // конца дня: наблюдения — на первом же шаге, рекомендации — на пятом, будущее — на
  // десятом. Судить их в конце значило складывать в одну очередь работу, которую можно
  // было сделать по дороге.
  //
  // ДВА ПРАВИЛА, БЕЗ КОТОРЫХ ЭТОГО ДЕЛАТЬ НЕЛЬЗЯ.
  // 1. Участник НЕ ЖДЁТ. Ответа не дожидаемся вовсе: запрос уходит и живёт своей жизнью
  //    на сервере, пока человек читает следующую реплику. Сорок секунд судьи он не видит.
  // 2. Сначала ответ должен доехать. Сохранение отложено на три секунды, и позвать судью
  //    раньше значило бы дать ему прочитать пустое место. Поэтому сначала sync, и только
  //    в его then — вызов.
  // Какой шаг какое задание открывает, решает БЭКЕНД (карта V2_JUDGE_AFTER_STEP): здесь
  // мы просто честно сообщаем, что участник зафиксировал. Шаг, на котором судить нечего,
  // получает в ответ «started: 0» и стоит ничего.
  function judgeStepSoon(key) {
    if (blockedByVersion || isDemo || !key || !window.imp.isApiConfigured()) return;
    sync().then(function () {
      try {
        var p = window.imp.callApi('judgeStep', { bib: session.bib, after: String(key) });
  ...
  function subst(text) {
    ...
    if (t.indexOf('{who}') >= 0 || t.indexOf('{title}') >= 0 || t.indexOf('{whoName}') >= 0 || t.indexOf('{а}') >= 0) {
      ...
      // ⚠ {а} — РОД ХОЗЯИНА ЗАЯВКИ (правка 21.09). «Понял{а}» превращается в «Понял» или
      // «Поняла», смотря кого участник отклонил: на выходе его догоняет живой человек, и
      // ответ в чужом роде — это не опечатка, а сообщение «мы не заметили, с кем вы
      // говорите». Подстановка общая, а не одно слово: она годится любому глаголу
      // прошедшего времени, который понадобится дальше. Род объявлен в js/backlog.js.
  ...
  function revealRun(host, done, clear) {
    ...
    var follow = function (n) {
      ...
      var pull = function () {
        ...
        // ⚠ ВЫСОКИЙ УЗЕЛ ТЯНЕМ ЗА ВЕРХ, А НЕ ЗА НИЗ (правка 11.09, поймано владельцем:
        // «при первом открытии сразу пролистывается весь список решений в конец»).
        // Правило «подтянуть низ» верно для реплики: она в несколько строк, и её низ —
        // это её конец. Для верстака оно бессмысленно: разбор заявок на живой странице
        // имеет высоту около 14 000 пикселей при колонке в 700, и «показать низ» значит
        // промотать двадцать заявок и встать на последней. Замер 11.09: после показа
        // колонка вставала на 14 789 из 15 115. То же правило уже действует ниже, в
        // подтяжке рабочей области («у верстака тянем ВЕРХ… поймано владельцем 07.08»),
        // но досюда оно не доехало — а показывает верстак именно этот код.
        // Вверх не тянем никогда: если человек сам ушёл читать выше, экран не отнимаем.
        if (nb.height > sb.height - pad) {
          var up = nb.top - (sb.top + 8);
          if (up > 0) sc.scrollTop += up;
          return;
        }
  ...
  // Какие верстаки уже показывали с начала: подтяжка верха делается один раз на шаг,
  // дальше колонка принадлежит участнику.
  var benchTopDone = {};
  var supportTab = 'case';
  ...
  function caseWideMove(on) {
    ...
    // ⚠ ВКЛАДКА ОБЯЗАНА ПЕРЕЖИТЬ ПЕРЕЕЗД (правка 11.09, поймано владельцем: «при открытии
    // кейса не открыта ни одна вкладка справа»). Узел кейса при переезде физически уходит
    // из панели в середину, а его язычок прячется — и если активной оставалась вкладка
    // «Кейс», панель гасла целиком: подсвечивать нечего, показывать нечего. Так бывает не
    // только при ручном ресайзе: порог 1361 проверяется в момент вызова, и если раскладка
    // на первом кадре ещё не устоялась (полоса прокрутки, зум, окно у самой границы),
    // переезд случается вторым заходом — уже после того, как вкладку выбрали. Держим
    // инвариант в одном месте: уехал кейс — открыты пометки.
    if (on && supportTab === 'case') setTab('marks');
  }
    ...
    var openWork = function () {
      ...
      if (sc2 && tgt) {
        var pull2 = function () {
          ...
          // ⚠ ВЫСОКИЙ ВЕРСТАК ПОКАЗЫВАЕМ С НАЧАЛА, А НЕ С КОНЦА (правка 11.09, поймано
          // владельцем: «при первом открытии сразу пролистывается весь список решений в
          // конец»). Причина не в верстаке, а в карточке вопроса внутри него: у разбора
          // заявок вопрос «почему выбрали именно такой состав» стоит ПОД двадцатью
          // заявками, это .mx-ansbox, и правило «подтянуть окно ответа в зону чтения»
          // честно тащило её к кромке — то есть проматывало весь список. Замер 11.09:
          // колонка вставала на 2 674 из 2 776, верх верстака оказывался на 2 543 пикселя
          // выше кромки. Для коротких верстаков (цель, письмо, печать) прежнее правило
          // верное и остаётся: там карточка видна вместе со всем верстаком. Правило здесь
          // одно и по смыслу: если верстак НЕ ВЛЕЗАЕТ в колонку, участник должен увидеть
          // его начало. Только при первом показе шага — дальше колонка принадлежит ему.
          var benchHost = now.querySelector('.mx-host');
          if (benchHost) {
            var bKey = 'bench:' + (benchHost.getAttribute('data-mech') || '') + ':' + state.cursor;
            var hb2 = benchHost.getBoundingClientRect();
            if (hb2.height > sb2.height && !benchTopDone[bKey]) {
              // ⚠ И ПОВТОРНО ЧЕРЕЗ ПОЛСЕКУНДЫ. Первый заход ставит колонку правильно —
              // замер 11.09 показывает 131 вместо 2 674, — но следом её кто-то уводит вниз
              // снова, и поймать кто не удалось: ни одна запись scrollTop, ни scrollTo, ни
              // scrollIntoView в этот момент не проходит через инструментацию, а сам узел
              // колонки к тому времени уже другой. Поэтому положение подтверждается вторым
              // заходом, как это уже сделано у плавной подтяжки ниже («второй замер после
              // того, как анимация должна была кончиться»). Флаг ставится только после
              // второго: до него шаг считается ещё не показанным.
              var align = function (last) {
                var host2 = now.querySelector('.mx-host');
                var col = el('talkScroll');
                if (!host2 || !col) return;
                var a = host2.getBoundingClientRect(), b = col.getBoundingClientRect();
                if (a.height <= b.height) return;
                var off = a.top - b.top;
                if (Math.abs(off) > 4) col.scrollTop += off;
                if (last) benchTopDone[bKey] = 1;
              };
              align(false);
              setTimeout(function () { align(true); }, 420);
              return;
            }
          }
