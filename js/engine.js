...
(function () {
  ...
  // ⚠ КАРТОЧКА ЗАЯВКИ, НА КОТОРОЙ НАСТОЯЛО ПРАВЛЕНИЕ (правка владельца 28.08). В окне
  // обмена Агеев называет заявку словами, а разбор двадцати позиций был получасом раньше:
  // без карточки шаг мерил бы память — что это за заявка и в чём её суть. Ровно то же
  // основание, по которому карточка стоит в разговоре у выхода.
  // Заявку выбирает то же правило, что подставляет её в реплику (js/backlog.js, forcedPick),
  // и с тем же признаком демо — иначе на витрине карточка и реплика разошлись бы.
  ...
  function windowBlock(act, locked) {
    ...
    d.innerHTML =
      ...
      (act.card === 'refused' ? refusedCardHtml()
        : act.card === 'forced' ? forcedCardHtml() : '') +
  ...
  function render() {
    ...
    for (var i = 0; i < route.length; i++) {
      ...
      if (past && sceneHasFold) { pending = []; continue; }
