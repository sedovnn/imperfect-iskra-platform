  ...
  function initWorkspace() {
    ...
    function buildQ1Block() {
      ...
      block.innerHTML =
        ...
        them('Брагин', { act: 'фыркает', speech: '«Да ничего они не построят. Наберут звёзд и утонут».' }) +
        them('Брагин', { act: 'поворачивается к вам', speech: '«Вот и рассудите: кто из нас прав — получится у них или нет? И если бы вы были на месте Даши, но в Nord Labs — что бы делали?»' }) +
    ...
    function buildQ2Block() {
      ...
      block.innerHTML =
        them('Брагин', (state.answer1 || '').trim().length >= 40
          ? { act: 'хмыкает', speech: '«Смотрите-ка, не пустой звук».' }
          : { act: 'пожимает плечами', speech: '«Ну, допустим».' }) +
        them('Даша', { act: 'ставит стакан', speech: '«Почему вы сделали именно такие выводы? И как думаете, какие ещё у них карты на руках?»' }) +
    ...
    function buildQ3Block() {
      ...
      block.className = 'chat';
      block.innerHTML =
        them('Девушка из отдела логистики «Меридиана»', { act: 'вполуха из очереди позади',
        ...
        them('Брагин', { act: 'провожает её взглядом', speech: '«Тут все друг у друга подсматривают, кофейня такая. Только вы про них ничего толком не знаете — ни цифр, ни планов. Откуда тогда всё это? Из головы или видели где-то?»' }) +
