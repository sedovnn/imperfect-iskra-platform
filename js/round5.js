  ...
  function initWorkspace() {
    ...
    function buildQ1Block() {
      ...
      block.innerHTML =
        them('Олег Брагин', { note: 'тот самый, из первой сотни', act: 'ловит взглядом ваш бейдж',
          speech: '«' + (pname() ? escapeHtml(pname()) + '? Консультант' : 'Консультант') + ', значит. Со стороны. Тогда рассудите — а то мы тут с Дарьей второй кофе спорим».' }) +
        ...
        them('Олег Брагин', { act: 'поворачивается к вам', speech: '«Вот и рассудите: кто из нас прав — получится у них или нет? И если бы вы были на месте Дарьи, но в Nord Labs — что бы делали?»' }) +
    ...
    function buildQ2Block() {
      ...
      block.innerHTML =
        ...
        them('Дарья', { act: 'ставит стакан', speech: '«Да, подожди ты. ' + (pname() ? escapeHtml(pname()) + ', почему' : 'Почему') + ' вы сделали именно такие выводы? И как думаете, какие ещё у них карты на руках?»' }) +
        (locked ? me(state.subdecisions)
                : inputBox('ga-subdec', 'Почему такие выводы и какие ещё у них карты на руках', state.subdecisions, 'ваш ответ Дарье', 'commitQ2Btn', 'Ответить'));
      if (!locked) {
