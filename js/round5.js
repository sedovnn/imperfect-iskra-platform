...
// участника не покрывает: Дарье (тимлид) спустили сверху «перебей офферы, удержи
...
// НЕ подсказывается: ни Дарья, ни камео её не проговаривают.
//
// Три шага ( id шагов прежние — q1/q2/q3):
//   q1  Брагин пересказывает Дарьину задачу и задаёт ОТКРЫТЫЙ вопрос («сработает
...
(function () {
  ...
  function initWorkspace() {
    ...
    // q1 — сцена (Брагин + Дарья) и ОТКРЫТЫЙ вопрос. Списка не просим: несколько
    // разных ходов должны появиться сами (ГА-1 2→3 «б»). Дарья НЕ проговаривает
    // сомнение «деньги ли это» — иначе переформулировка задачи (верхний уровень)
    // была бы подсказана; спор Брагина и Дарьи сигналит только «это обсуждаемо».
    ...
    function buildQ1Block() {
      ...
      block.innerHTML =
        them('Олег Брагин', { note: 'тот самый, из первой сотни', act: 'ловит взглядом ваш бейдж',
          speech: (pname() ? escapeHtml(pname()) + '? Консультант' : 'Консультант') + ', значит. Со стороны. Кстати вовремя — мы тут с Дарьей второй кофе стоим.' }) +
        them('Дарья', { note: 'тимлид одной из продуктовых команд', act: 'не отрываясь от стакана',
          speech: 'У меня половина команды переписывается с Nord Labs — сама видела, как двое ходили на кофе к их рекрутёру. Сверху спустили бюджет: перебей офферы, удержи людей. Мне до завтра решить, что я с этим делаю.' }) +
        them('Олег Брагин', { act: 'фыркает', speech: 'Деньги дали — бери и держи, чего тут думать.' }) +
        // Идиома кейса, отданная персонажу как жаргон, без расшифровки: расшифровка
        // живёт в кейсе («на одном этаже обсуждают продукт, на другом обязательства»).
        // Стоит там, где вывод участнику уже не подсказывает — до его ответа.
        them('', { act: 'разводит руками', speech: 'Это у нас «два этажа» называется. Я-то с первого.' }) +
        them('Дарья', { act: 'поворачивается к вам, игнорируя Брагина', speech: 'Что бы вы сделали на моём месте? Мне не рассудить нас надо — мне завтра с людьми разговаривать.' }) +
        (locked ? me(state.answer1)
                : inputBox('s2-rationale', 'Что бы вы сделали на месте Дарьи', state.answer1, 'ваш ответ Дарье', 'commitQ1Btn', 'Ответить'));
      if (!locked) {
        ...
        block.querySelector('#commitQ1Btn').addEventListener('click', function () {
          ...
          if (!state.answer1.trim()) {
            window.imp.confirm('Ничего не ответить Дарье — так и зафиксируем?', { confirmLabel: 'Промолчать', cancelLabel: 'Вернуться к ответу' })
    ...
    function buildQ2Block() {
      ...
      block.innerHTML =
        ...
        them('Дарья', { act: 'ставит стакан', speech: 'А если шире — не про мою команду, а вообще: как нам людей удерживать? Идей у меня ноль, у Брагина одна. И скажите, почему вы сделали именно такие выводы.' }) +
        (locked ? me(state.subdecisions)
                : inputBox('ga-subdec', 'Как удерживать людей и почему такие выводы', state.subdecisions, 'ваш ответ Дарье', 'commitQ2Btn', 'Ответить'));
      if (!locked) {
