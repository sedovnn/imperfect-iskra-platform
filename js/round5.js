  ...
  function initWorkspace() {
    ...
    function buildQ1Block() {
      ...
      block.innerHTML =
        ...
        '<p class="s2-ageev"><b>Даша</b>, тимлид одной из продуктовых команд, не отрываясь от стакана: «Да что спорить. У меня половина команды переписывается с Nord Labs — сама видела, как двое ходили „на кофе“ к их рекрутеру. Сверху спустили бюджет: перебей офферы, удержи людей. „Просто заматчи, и всё“.»</p>' +
        '<p class="s2-ageev"><b>Брагин</b> поворачивается к вам: «Вот. Вы со стороны — как думаете: сработает так, как ей велят? Что бы вы на её месте держали в голове?»</p>' +
        '<textarea class="s2-rationale" aria-label="Сработает ли план с офферами и что бы вы держали в голове" rows="5" placeholder="ваш ответ"' + (locked ? ' disabled' : '') + '>' + escapeHtml(state.answer1) + '</textarea>' +
    ...
    function buildQ2Block() {
      ...
      block.innerHTML =
        ...
        '<p class="s2-ageev"><b>Брагин</b> делает глоток: «А сами-то где колебались? Наверняка что-то перебрали по дороге — и что-то отбросили».</p>' +
    ...
    function buildQ3Block() {
      ...
      block.className = 's2-block';
      block.innerHTML =
        '<p class="s2-ageev"><b>Женщина с логистики Меридиана</b>, вполуха из очереди позади: «У нас в позапрошлом году та же песня была — тоже голову ломали, чем людей держать». Забирает свой стакан, уходит к лифтам.</p>' +
        '<p class="s2-ageev"><b>Брагин</b> провожает её взглядом: «Тут все друг у друга подсматривают, кофейня такая. А вы своё — откуда взяли? Из головы, из жизни?»</p>' +
