...
(function () {
  ...
  var CHEATSHEET = {
    ...
    // Блока «Деньги» здесь больше нет, и это не сокращение, а снятая утечка.
    // Он называл обе позиции правления («Крепость» — 180, «Вторая кривая» — 230)
    // и обе рамки — при том что панель доступна с первой минуты, а Агеев
    // произносит развилку только в кабинете, во втором разговоре. То есть
    // участник мог прочитать структуру задачи раньше, чем её задали: кейс
    // скрывает развилку намеренно (правило версии v5), а шпаргалка выдавала.
    // Обе рамки и так звучат там, где нужны: ресурс года — в кейсе (раздел 2 и
    // сноска (г) П1) и вслух у Агеева, рамка консорциума — в кейсе (раздел 3).
  };
  function rows(list) {
  ...
  // Загрузка пакета — ОДНА на всю платформу: этой же функцией движок наполняет
  // экран чтения в начале дня (js/engine.js, акт kind:'case'). Иначе экран и
  // панель тянули бы кейс двумя разными кусками кода и рано или поздно разошлись
  // бы в том, что считать кейсом.
  ...
  // Адрес задаёт страница (data-case-src); по умолчанию — кейс v6.
