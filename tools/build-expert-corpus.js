...
// Отпечаток §10 — по нормализованному тексту всех десяти блоков, чтобы правка
// пробелов версию не двигала, а правка формулировки двигала.
CORPUS_VERSION = CORPUS_VERSION_BASE + '.' + require('crypto').createHash('sha1')
  .update(CODES.map(function (c) { return abilityChunks[c]; }).join('\n').replace(/\s+/g, ' ').trim())
